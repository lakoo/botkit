...
function Botkit(configuration) {
    ...
    botkit.utterances = {
        yes:    '(?:(?<!不)是|(?<!不)對|(?<!不)要|(?<!唔)係|(?<!唔)啱|(?<!不)是|(?<!不)对|(?<!不)要|yes|yea|yup|yep|ya|sure|ok|y|yeah|yah)',
        no:     '(?:不|唔|不|no|nah|nope|n)',
        quit:   '(?:quit|cancel|end|stop|done|exit|nevermind|never mind)',
    };
        ...
        for (var t = 0; t < tests.length; t++) {
            if (message.text) {
                ...
                if (typeof(tests[t]) == 'string') {
					match = pregex.exec(message.text, tests[t], 'i');
                } else {
					match = pregex.exec(message.text, tests[t]);
                }
